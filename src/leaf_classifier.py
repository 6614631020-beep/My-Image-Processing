...
import numpy as np
from glob import glob # การอ่านชื่อไฟล์ทั้งหมด
...
if __name__ == "__main__" :
    ...
    # อ่านรูปหลายๆ รูป
    input_files1 = glob(DATABASE_PATH + "1/" + "*")
    input_files2 = glob(DATABASE_PATH + "2/" + "*")
    input_files = input_files1 + input_files2
    print(input_files)

    for f in input_files: # ลูปให้อ่านรูปทุกรูป
    # - Read image / อ่านรูป
        input_img = cv.imread(f)
        rgb_img = cv.cvtColor(input_img, cv.COLOR_BGR2RGB)

        # - Color Redius Segmentation
        ...
        # - Mrophological Processing
        stre = skmorph.disk(9)
        morph_img = removeFragments(seg_img, thresh_ratio=0.05) # ลบรอบๆ ปรับขนาด
        morph_img = fillHoles(morph_img) # เพิ่มไม่ให้มีช่องว่าง
        morph_img = cv.morphologyEx(morph_img, cv.MORPH_CLOSE, stre)
        morph_img = fillHoles(morph_img)

        # - Feature Extraction
        _, eccen = regionBasedFeatures(morph_img, "eccentricity") # ระบุลักษณะหลังจากทำ Segmentation เสร็จแล้ว eccentricity
        # print(eccen[0])
